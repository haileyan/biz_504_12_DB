...
--DB에서 어떤 일을 수행하기 전에 
--가장 먼저 저장소를 생성해야 한다.
--저장소 중에서 표준 DB인 대표 저장소인 TABLE을 생성
CREATE TABLE tbl_addr(
    name CHAR(50),  --50글자 범위의 글자를 저장
    age NUMBER(10,1), --정수부 10자리, 소수부 1자리
    addr CHAR(50)
);
DROP TABLE tbl_addr ;
...
CREATE TABLE tbl_addr (
    name VARCHAR2(50), --50글자 범위의 문자를 저장하되
        -- 만약 글자수가 50글자가 안 되면 남은 공간은 압축해서 저장
    age NUMBER, -- 정수 38자리
    addr VARCHAR2(125)
);
...
-- 가급적 최대 자릿수를 고려하여 값을 지정해 주는 것이 좋다.
